...
const EditBranchScreen = ({ route, navigation }) => {
    ...
    const SaveBranch = () => {
        ...
        fetch(`${IP_ADDRESS}/save_branch_information?name=${branchName}&location=${branchLocation}&phone=${phone}&id=${branch._id}`)
            .then(res => {
                if (res.status == 200) {
                    return res.json()
                } else {
            ...
            .then(res => {
                ...
                // Show success alert
                Alert.alert('Success', 'Branch information updated successfully.');
